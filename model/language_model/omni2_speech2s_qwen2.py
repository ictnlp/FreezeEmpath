...
from model.language_model.omni2_speech_qwen2 import Omni2SpeechQwen2ForCausalLM
from model.speech_generator.builder import build_speech_generator
from model.speech_generator.generation import GenerationWithTTS
...
class Omni2Speech2SQwen2ForCausalLM(Omni2SpeechQwen2ForCausalLM, GenerationWithTTS):
    ...
    def initialize_speech_generator(self, model_args):
        ...
        self.config.speech_generator_arch = getattr(model_args, 'speech_generator_arch', 'qwen')
        self.config.tts_loss_weight = getattr(model_args, 'tts_loss_weight', 1.0)
        self.config.num_sample_turns_s2s = getattr(model_args, "num_sample_turns_s2s", 0)
        self.config.unit_vocab_size = getattr(model_args, 'unit_vocab_size', 4096)
        self.config.tts_max_length = getattr(model_args, 'tts_max_length', 2048)
        self.config.stream_params = getattr(model_args, 'stream_params', "(1,1)")
        self.config.add_token_embeddings = getattr(model_args, 'add_token_embeddings', False)
        self.config.add_gate_fusion = getattr(model_args, 'add_gate_fusion', False)
        self.config.speech_generator_proj_type = getattr(model_args, 'speech_generator_proj_type', 'linear')
        self.config.tts_ffn_dim_size = getattr(model_args, 'tts_ffn_dim_size', -1)
    ...
    def forward(
        ...
        if self.training:
            if self.tune_speech_generator_only:
                ...
                loss = self.speech_generator(lm_output['hidden_states'][-1], labels, tgt_units).loss
            else:
                lm_output = super(Omni2SpeechQwen2ForCausalLM, self).forward(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                ...
                tts_loss = self.speech_generator(lm_output['hidden_states'][-1], labels, tgt_units).loss
                loss = lm_loss + tts_loss * self.config.tts_loss_weight
        else:
            lm_output = super(Omni2SpeechQwen2ForCausalLM, self).forward(
                input_ids=input_ids,
                attention_mask=attention_mask,
